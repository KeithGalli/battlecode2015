...
import strikerBasicv2.DataCache;
...
public class NetworkPathing{
	...
	public static void initNavSystem(RobotController rc) throws GameActionException{
		width = rc.getMapHeight();
		height = rc.getMapWidth();
		waypointID = new int[width+4][height+4];
		...
		System.out.println("done-networkmaker");
...
//		displayArray(visual);
	...
	private static void setWaypointID(int x, int y, int voidID, int waypointType){
		waypointID[x+2][y+2] = voidID*1000+waypointType;
	}
	...
	private static ArrayList<MapLocation> findOutermostVoids(ArrayList<MapLocation> contiguousVoids,ArrayList<MapLocation> outermost,int voidID) throws GameActionException{
		...
		for(MapLocation current:outermost){
			...
			if (waypointChecker==1100){
				setWaypointID(current.x, current.y, voidID, 1);
			}
			else if (waypointChecker==1001){
				setWaypointID(current.x, current.y, voidID, 2);
			}
			else if (waypointChecker==110){
				setWaypointID(current.x, current.y, voidID, 3);
			}
			else if (waypointChecker==11){
				setWaypointID(current.x, current.y, voidID, 4);
			}
