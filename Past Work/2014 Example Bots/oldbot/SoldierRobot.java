...
public class SoldierRobot extends BaseRobot {
	...
	public SoldierRobot(RobotController rc) throws GameActionException {
		...
		int jobnum = BroadcastSystem.read(1);
		...
		rc.setIndicatorString(0, Integer.toString(jobnum));
...
//		}
	...
	public void defenderCode() throws GameActionException{
		...
		if (DataCache.currentLocation.distanceSquaredTo(guarding)<=2){
			arrived = true;
		}
		
		if (DataCache.currentLocation.distanceSquaredTo(guarding)<=25){
			inPosition = true;
		}
		
		if (arrived){
			if (!inPosition){
				Direction dir = DataCache.currentLocation.directionTo(guarding);
			...
			if (trialLoc.distanceSquaredTo(guarding)<25){
				NavSystem.simpleNav(dir);
			}
			else{
				holdPosition();
	...
	private void towerCode(MapLocation towerSite) throws GameActionException {
		if (DataCache.currentLocation.equals(towerSite)){
			buildTower();
		}
		else if (dataDownloaded){
			NavSystem.blindNav(towerSite);
		}
		else{
			NavSystem.snailNav(DataCache.currentLocation.directionTo(towerSite));
		}
	...
	private void specificPastrCode(MapLocation pastrSite) throws GameActionException {
